...
if(!$result || mysqli_num_rows($result) == 0 ){
  ...
  if($addOffer = $connection->prepare("INSERT INTO offer (name, provider, type, price, priceType, data, dataUnit, calls, fiber, description) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
    ...
    $addOffer->execute();
    $addOffer->close();
    write_log("Insertado nueva oferta con Nombre: $name, Proveedor: $provider, Tipo: $type, Precio: $price, Tipo_Precio: $priceType, Datos: $default_data $default_dataUnit, Llamadas: $default_calls, Fibra: $default_fiber, Descripción: $description.");
